Title: GitHub - himanshudongre/smriti

URL Source: https://github.com/himanshudongre/smriti

Markdown Content:
**Code has Git. Multi-agent reasoning does not.**

Smriti is version control for project reasoning state — versioned, structured, branchable snapshots of _what was decided_, _what's still open_, and _what each agent is doing right now_. So multiple coding agents can coordinate on the same codebase without overwriting each other's thinking.
...
[](https://github.com/himanshudongre/smriti#the-git-analogy)
Git preserves _code history_: what changed, when, by whom, on which branch. Smriti preserves _project reasoning_: what was decided, what's still open, what each agent is doing right now, and how the recorded state compares to the live repo.
...
[](https://github.com/himanshudongre/smriti#why-markdown-handoffs-arent-enough)
...
The fundamental difference: markdown is _prose_. Smriti is _structured, versioned, queryable state_. Markdown describes what you were thinking; Smriti lets the next session pick up where you left off — without re-reading prose, without two agents redoing the same work, without lying about the repo.

* * *

## Getting started

[](https://github.com/himanshudongre/smriti#getting-started)
...
[](https://github.com/himanshudongre/smriti#1-install-local-first)
...
Local mode stores state in SQLite at `~/.smriti/smriti.db`. For a shared/team setup with Postgres, see [Shared / team mode](https://github.com/himanshudongre/smriti#shared--team-mode-postgres) below.

For the chat UI, run `make dev-frontend` in a separate terminal and open [http://localhost:5173](http://localhost:5173/).

### 2. Confirm install — `smriti doctor`

[](https://github.com/himanshudongre/smriti#2-confirm-install--smriti-doctor)
...
[](https://github.com/himanshudongre/smriti#3-see-smriti-at-work--smriti-quickstart)

smriti quickstart

Seeds a `smriti-demo` Space — one finished mini-project (a rate-limiting feature built by two agents, with a branch explored and dropped) — and prints a ~3-minute guided walkthrough. Works without API keys — quickstart seeds pre-built checkpoints, no live extraction. (For live `--extract` you need a real provider; see [Provider configuration](https://github.com/himanshudongre/smriti#provider-configuration-llm-backed-features).) Clean up with `smriti quickstart --remove`.

### 4. Attach your own project — `smriti init`

[](https://github.com/himanshudongre/smriti#4-attach-your-own-project--smriti-init)
...
**Multiple projects.** Attachment is per-project-directory, not global. Everything `smriti init` writes — `.smriti.json`, the SessionStart hook in `.claude/settings.json`, and the skill packs (`.claude/skills/smriti/SKILL.md` and `AGENTS.md`) — lives _inside the project directory_. Run `smriti init` once per project; the files for `~/code/project-a` and `~/code/project-b` never see each other. A Claude Code or Codex session opened in `~/code/project-a` automatically lands on space `project-a`; the same agent opened in `~/code/project-b` lands on `project-b`. The MCP server registration (the JSON block above) is machine-wide but stateless — every MCP call passes `space="..."` explicitly, so the server has no "current space" of its own.

### 5. Daily workflow — no `<space>` needed

[](https://github.com/himanshudongre/smriti#5-daily-workflow--no-space-needed)
...
[](https://github.com/himanshudongre/smriti#shared--team-mode-postgres)
...
[](https://github.com/himanshudongre/smriti#what-you-get)
...
[](https://github.com/himanshudongre/smriti#four-surfaces-one-core)
...
3.   **Agent skill pack** — versioned instructions teaching agents _when_ to checkpoint (and critically _when not to_), when to fork, how to detect drift, and how to pick complementary work. Install once per project.
...
[](https://github.com/himanshudongre/smriti#what-smriti-is-not)
...
[](https://github.com/himanshudongre/smriti#provider-configuration-llm-backed-features)
...
[](https://github.com/himanshudongre/smriti#works-with-no-key)
...
[](https://github.com/himanshudongre/smriti#require-a-real-provider)
...
[](https://github.com/himanshudongre/smriti#configure-a-provider)
...
Extraction is a relatively lightweight structured-output task and is usually handled well by cost-efficient or local OpenAI-compatible models. That said, _quality varies by model_ — validate with `smriti checkpoint create --extract --dry-run` on a representative document before relying on a small model for real work.
...
[](https://github.com/himanshudongre/smriti#mock-mode)
...
[](https://github.com/himanshudongre/smriti#built-with-smriti)
...
[](https://github.com/himanshudongre/smriti#what-it-looks-like)
### Project timeline (dashboard)

[](https://github.com/himanshudongre/smriti#project-timeline-dashboard)
[![Image 1: Smriti LineagePage dashboard](https://github.com/himanshudongre/smriti/raw/main/docs/assets/lineage-dashboard.png)](https://github.com/himanshudongre/smriti/blob/main/docs/assets/lineage-dashboard.png)

_Real project timeline from building Smriti with Smriti: summary panel, checkpoint cards with author badges, note indicators, needs-attention signal._

### Agent-facing state and metrics (CLI)

[](https://github.com/himanshudongre/smriti#agent-facing-state-and-metrics-cli)
[![Image 2: CLI state and metrics](https://github.com/himanshudongre/smriti/raw/main/docs/assets/cli-state-and-metrics.png)](https://github.com/himanshudongre/smriti/blob/main/docs/assets/cli-state-and-metrics.png)

_What every agent reads at session start (`smriti state --compact`) and the project health KPIs (`smriti metrics`)._

### Structured checkpoint detail

[](https://github.com/himanshudongre/smriti#structured-checkpoint-detail)
[![Image 3: Checkpoint detail with structured tasks and notes](https://github.com/himanshudongre/smriti/raw/main/docs/assets/checkpoint-detail.png)](https://github.com/himanshudongre/smriti/blob/main/docs/assets/checkpoint-detail.png)

_The autonomy milestone checkpoint — structured tasks with intent badges, task IDs, and a founder milestone note annotating the first clean autonomous complementary-work proof._

* * *

## Core concepts

[](https://github.com/himanshudongre/smriti#core-concepts)
...
[](https://github.com/himanshudongre/smriti#context-modes)
...
[](https://github.com/himanshudongre/smriti#current-limitations)
...
[](https://github.com/himanshudongre/smriti#tech-stack)
FastAPI · SQLAlchemy · PostgreSQL / SQLite · React + TypeScript + Vite

## Docker (Postgres mode helpers)

[](https://github.com/himanshudongre/smriti#docker-postgres-mode-helpers)
...
[](https://github.com/himanshudongre/smriti#try-the-demo)
